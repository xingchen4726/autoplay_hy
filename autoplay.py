...
def end():
    while True:
        try:
            # 尝试找到退出图像
            endlocation = pyautogui.locateOnScreen('end.png', confidence=confidence_threshold)
            if endlocation:
...
def run():
    while True:
        ...
        # 使用函数简化代码
        check_and_move(1100, 500, 700, 900, "down")
        
        check_and_move(1300, 700, 700, 900, "left")
        check_and_move(1100, 900, 700, 900, "up")
        check_and_move(900, 700, 700, 900, "right")
        '''
        x=700
        y=900
        if pyscreeze.pixelMatchesColor(1100, 500, target_color, tolerance<tolerance):
            pyautogui.moveTo(x, y, duration=0.2)
            pyautogui.dragTo(x, y+100, duration=0.6, button='left')
        if pyscreeze.pixelMatchesColor(1100, 900, target_color, tolerance<tolerance):
            pyautogui.moveTo(x, y, duration=0.2)
            pyautogui.dragTo(x, y-100, duration=0.6, button='left')
        if pyscreeze.pixelMatchesColor(1300, 700, target_color, tolerance<tolerance):
            pyautogui.moveTo(x, y, duration=0.2)
            pyautogui.dragTo(x-100, y, duration=0.6, button='left')
        if pyscreeze.pixelMatchesColor(900, 700, target_color, tolerance<tolerance):
            pyautogui.moveTo(x, y, duration=0.2)
            pyautogui.dragTo(x+100, y, duration=0.6, button='left')
        print("控制移动")
        '''
        ...
        try:
            # 尝试找到继续图像
            continuelocation = pyautogui.locateOnScreen('continue.png', confidence=confidence_threshold)
            if continuelocation:
...
while  K<10:
    try:
        # 尝试找到开始图像
        startlocation = pyautogui.locateOnScreen('start.png', confidence=confidence_threshold)
        if startlocation:
